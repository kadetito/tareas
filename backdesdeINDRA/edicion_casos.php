<?php session_start();
      ...
      require('lib/paginador.php');
        if(($_GET['getId']=='') || ($_GET['getId']=='000000')){header('Location: '.$path.'/index');} else {$getId=$_GET['getId'];$sqlGeneral = "SELECT * FROM suit_suites WHERE id = ".$_GET['getId']." ";}
        ...
        $urlpagina6 = '<li class="breadcrumb-item"><a href="'.$path.'/'.$lengua.'/'.$rowGeneral['id'].'/c/casosprueba">'.$titulo_seccion.'</a></li>';
        $urlpagina7 = '<li class="breadcrumb-item active">'.$TXT_editarCasoPrueba.'</li>';
...
<div class="card mb-4">
									<div class="card-block">
										<h3 class="card-title"><?=$TXT_editarCasoPrueba;?></h3>
			...
			echo '<div class="row">';
				...
				recuperarArrayCasos($path,$lengua,$getId,$recup,$valor,$tabla);
				echo '</div>';
			echo '</div>';
			?>	
										
			</form>				
			<?php guardarMasivoCasos($path,$lengua,$getId);?>					

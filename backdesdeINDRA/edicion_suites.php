<?php session_start();
      ...
      require('lib/paginador.php');
        if(($_GET['getId']=='') || ($_GET['getId']=='000000')){header('Location: '.$path.'/index');} else {$getId=$_GET['getId'];$sqlGeneral = "SELECT * FROM buil_builds WHERE id = ".$_GET['getId']." ";}
        ...
        $urlpagina1 = '<li class="breadcrumb-item"><a href="'.$path.'/'.$lengua.'/'.$rowGeneral['id_proyecto'].'/gestion-proyectos">'.$titulo_seccion1.'</a></li>';
        $urlpagina2 = '<li class="breadcrumb-item"><a href="'.$path.'/'.$lengua.'/'.$rowGeneral['id_proyecto'].'/v/builds">'.$titulo_seccion2.'</a></li>';
        $urlpagina4 = '<li class="breadcrumb-item"><a href="'.$path.'/'.$lengua.'/'.$_GET['getId'].'/p/suites">'.$titulo_seccion.'</a></li>';
				$urlpagina5 = '<li class="breadcrumb-item active">'.$TXT_editarSuite.'</active>';
?><!DOCTYPE html>
<html lang="<?=$lengua;?>">
<head>
...
<div class="card mb-4">
									<div class="card-block">
										<h3 class="card-title"><?=$TXT_editarSuite;?></h3>
										...
										<h6 class="card-subtitle mb-2 text-muted"><?=$TXT_AquieditarSuite;?></h6>
			...
			echo '<div class="row">';
				...
				recuperarArraySuites($path,$lengua,$getId,$recup,$valor,$tabla);
				echo '</div>';
			echo '</div>';
			?>	
										
			</form>				
			<?php guardarMasivoSuite($path,$lengua,$getId);?>					
